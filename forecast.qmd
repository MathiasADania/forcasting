...
header-includes:
  ...
  #- \AtBeginDocument{\includepdf[pagses={1}]{Forside.pdf}} # Indsæt forside som første side
...
Testresultaterne fremgår af @sec-bilag-a.
...
jernbanedata %>%
  ...
  labs(title = "Differentieringstrin – International trafik i alt",
       y = NULL, x = NULL)
```
Figuren viser fire differentieringstrin for International trafik i alt. Sæsondifferentiering (D=1, lag=4) fjerner det periodiske mønster og 
...
afskæres derimod efter lag 4, hvilket peger på et sæsonelt AR(1)-led 
og understøtter en SAR(1)-specifikation.
...
Fejlmål for SNAIVE-benchmarken fremgår af Bilag B. @sec-bilag-b.
...
Ljung-Box testoutput fremgår af Bilag C. @sec-bilag-c
...
```
Residualerne ligger tæt på nul uden systematiske mønstre og ACF er 
inden for konfidensintervallet. Histogrammet er tilnærmelsesvist
...
Residualerne er overordnet hvid støj, men med et kraftigt coronaudsving i 2020 der skævvrider histogrammet. ACF viser ingen signifikante spike.

####Over Storbælt
Residualerne er centrerede omkring nul med et markant dyk i 2020, der ligeledes skævvrider histogrammet. ACF er inden for konfidensintervallet på alle lags.
...
# Bilag A: Stationaritetstest {#sec-bilag-a -}

```{r}
#| label: tbl-kpss-bilag
#| eval: true
jernbanedata %>%
  features(log(x1000_passagerer), unitroot_kpss) %>%
  kbl(caption = "KPSS-test — H0: stationær",
...
# Bilag B: SNAIVE benchmark {#sec-bilag-b -}

```{r}
#| label: benchmark-bilag
#| eval: true
#| results: asis
kør_begge_tbl(function(data, titel) {
  lav_stretch(data) %>%
...
# Bilag C: Ljung-Box tests {#sec-bilag-c -}
...
for (serie in c("International trafik i alt", "Over Storebælt")) {
  augment(fit_ets) %>%
    ...
    kbl(caption = paste0("Ljung-Box ETS – ", serie, " – Inkl. corona"),
...
for (serie in c("International trafik i alt", "Over Storebælt")) {
  augment(fit_ets_corona) %>%
    filter(.model == "Auto", key == serie) %>%
    features(.innov, ljung_box, lag = 8, dof = 0) %>%
    kbl(caption = paste0("Ljung-Box ETS – ", serie, " – Uden corona"),
        digits = 2, booktabs = TRUE) %>%
    kable_styling(latex_options = c("striped", "hold_position")) %>%
    print()
  cat("\n\n")
...
for (serie in c("International trafik i alt", "Over Storebælt")) {
  ...
  augment(fit_arima) %>%
    ...
    kbl(caption = paste0("Ljung-Box ARIMA – ", serie, " – Inkl. corona"),
        digits = 2, booktabs = TRUE) %>%
    kable_styling(latex_options = c("striped", "hold_position")) %>%
    print()
  cat("\n\n")
...
for (serie in c("International trafik i alt", "Over Storebælt")) {
  ...
  augment(fit_arima_corona) %>%
    filter(.model == "Auto", key == serie) %>%
    features(.innov, ljung_box, lag = 8, dof = dof) %>%
    kbl(caption = paste0("Ljung-Box ARIMA – ", serie, " – Uden corona"),
        digits = 2, booktabs = TRUE) %>%
    kable_styling(latex_options = c("striped", "hold_position")) %>%
    print()
  cat("\n\n")
